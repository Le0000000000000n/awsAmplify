...
import { createPortfolio, deletePortfolio, getAllocation, assetRemove, portfolioPerformance } from './portfolio.js';
...
app.post('/portfolio/assets', async (req, res) => {
    ...
    if (!userId || !assets || assets.length == 0) {
        return res.status(400).json({ error: 'Missing required fields (must not be empty)' });
    }
...
app.get('/stock/:symbol/history', async (req, res) => {
    try {
        const { historicalDataResponse, s3Location } = await getHistroicalStockData(req.params.symbol, req.query);
